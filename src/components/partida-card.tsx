...
export default function PartidaCard({ partida }: PartidaCardProps) {
  ...
  return (
    <Card
      className="mb-4 p-4 flex flex-row justify-between items-center cursor-pointer"
    ...
    >
      <div className="flex gap-2 md:gap-4 items-center">
        ...
        <h2 className="text-sm md:text-2xl font-bold">
          {format(partida.dataPartida.toDate(), "dd/MM/yyyy")}
        </h2>
      </div>
      ...
      <div>
        {artilheiroName ? (
          <div className="text-xs md:text-lg flex items-center flex-col gap-2 md:flex-row">
            ...
            <span className="md:font-semibold">{artilheiroName} ({artilheiro?.gols} gols)</span>
          </div>
        ) : (
